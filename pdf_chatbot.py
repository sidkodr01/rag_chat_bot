...
CHUNK_SIZE = 300
CHUNK_OVERLAP = 50
MAX_TOKENS = 8192  # Adjusted to match model's token limit
...
def process_pdf(file_path):
    ...
    if not content:
        raise ValueError("No content could be read from the PDF file. Ensure the file is not empty or corrupted.")
...
if __name__ == "__main__":
    print("Welcome to the Enhanced RAG Pipeline for PDF Documents.")
    
    while True:
        file_path = input("Please enter the path to the PDF document you want to query (or 'quit' to exit): ")
        ...
        try:
            print("Processing PDF document content...")
            texts = process_pdf(file_path)
            
            if not texts:
                print("No content found in the PDF document. Please try a different file.")
                continue
            ...
            qa = RetrievalQA.from_chain_type(
                llm=llm,
                chain_type="stuff",  # Consider "map_reduce" or "refine" for larger documents
